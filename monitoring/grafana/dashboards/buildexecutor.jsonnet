...
{
  dashboards+:: {
    'buildexecutor.json': simpledash.dashboard(
      title='BuildExecutor',
      templates=[
        simpledash.template(
        ...
        simpledash.row(
          ...
          panels=[
            simpledash.graph(
              ...
              targets=[
                simpledash.graphTarget(
                  expr='kubernetes_service:buildbarn_builder_build_executor_operations:irate1m{kubernetes_service=~"$kubernetes_service",%s="$cluster"}' % $._config.clusterLabel,
            ...
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le_stage:buildbarn_builder_build_executor_duration_seconds_bucket:irate1m{kubernetes_service=~"$kubernetes_service",stage="%s",%s="$cluster"}) by (le)' % [stage, $._config.clusterLabel],
                ),
              ],
            )
            ...
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le:buildbarn_builder_build_executor_virtual_execution_duration_bucket:irate1m{kubernetes_service=~"$kubernetes_service",%s="$cluster"}) by (le)' % $._config.clusterLabel,
                ),
              ],
            ),
          ]
        ...
        simpledash.row(
          ...
          panels=[
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le:buildbarn_builder_build_executor_posix_%s_bucket:irate1m{kubernetes_service=~"$kubernetes_service",%s="$cluster"}) by (le)' % [r[1], $._config.clusterLabel],
                ),
              ],
            )
            for r in [
              ['CPU user time', 'user_time', simpledash.unitSeconds, 1 / 2],
              ['CPU system time', 'system_time', simpledash.unitSeconds, 1 / 2],
        ...
        simpledash.row(
          ...
          panels=[
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le:buildbarn_builder_build_executor_file_pool_%s_bucket:irate1m{kubernetes_service=~"$kubernetes_service",%s="$cluster"}) by (le)' % [r[1], $._config.clusterLabel],
                ),
              ],
            )
            for r in [
            ...
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le_operation:buildbarn_builder_build_executor_file_pool_operations_%s_bucket:irate1m{kubernetes_service=~"$kubernetes_service",operation="%s",%s="$cluster"}) by (le)' % [r[1], r[2], $._config.clusterLabel],
                ),
              ],
            )
            for r in [
