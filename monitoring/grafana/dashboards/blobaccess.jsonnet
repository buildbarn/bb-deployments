...
{
  dashboards+:: {
    'blobaccess.json': simpledash.dashboard(
      title='BlobAccess',
      templates=[
        simpledash.template(
      ...
      rows=[
          ...
          create_graph=function(title, storageType, width) simpledash.graph(
            ...
            targets=[
              simpledash.graphTarget(
                expr='sum(backend_type_kubernetes_service_operation_storage_type:buildbarn_blobstore_blob_access_operations_started:irate1m{backend_type=~"$%s_backend_type",kubernetes_service=~"$kubernetes_service",storage_type="%s",%s="$cluster"}) by (operation)' % [storageType, storageType, $._config.clusterLabel],
        ...
        show_row(
          ...
          create_graph=function(title, storageType, width) simpledash.graph(
            ...
            targets=[
              simpledash.graphTarget(
                expr='sum(backend_type_grpc_code_kubernetes_service_storage_type:buildbarn_blobstore_blob_access_operations_duration_seconds_count:irate1m{backend_type=~"$%s_backend_type",kubernetes_service=~"$kubernetes_service",storage_type="%s",%s="$cluster"}) by (grpc_code)' % [storageType, storageType, $._config.clusterLabel],
          ...
          create_graph=function(title, storageType, width) simpledash.heatmap(
            ...
            targets=[
              simpledash.heatmapTarget(
                expr='sum(backend_type_kubernetes_service_le_operation_storage_type:buildbarn_blobstore_blob_access_operations_duration_seconds_bucket:irate1m{backend_type=~"$%s_backend_type",kubernetes_service=~"$kubernetes_service",operation="%s",storage_type="%s",%s="$cluster"}) by (le)' % [storageType, operation, storageType, $._config.clusterLabel],
              ),
            ],
          ),
        )
        ...
        show_row(
          ...
          create_graph=function(title, storageType, width) simpledash.heatmap(
            ...
            targets=[
              simpledash.heatmapTarget(
                expr='sum(backend_type_kubernetes_service_le_operation_storage_type:buildbarn_blobstore_blob_access_operations_blob_size_bytes_bucket:irate1m{backend_type=~"$%s_backend_type",kubernetes_service=~"$kubernetes_service",operation="%s",storage_type="%s",%s="$cluster"}) by (le)' % [storageType, operation, storageType, $._config.clusterLabel],
              ),
            ],
          ),
        )
        ...
        show_row(
          ...
          create_graph=function(title, storageType, width) simpledash.heatmap(
            ...
            targets=[
              simpledash.heatmapTarget(
                expr='sum(backend_type_kubernetes_service_le_storage_type:buildbarn_blobstore_blob_access_operations_find_missing_batch_size_bucket:irate1m{backend_type=~"$%s_backend_type",kubernetes_service=~"$kubernetes_service",storage_type="%s",%s="$cluster"}) by (le)' % [storageType, storageType, $._config.clusterLabel],
              ),
            ],
          ),
        ),
