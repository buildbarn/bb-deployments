...

{
  ...
  dashboards+:: {
    'fuse.json': simpledash.dashboard(
      title='FUSE',
      templates=[
        simpledash.template(
            ...
            simpledash.graph(
              ...
              targets=[
                simpledash.graphTarget(
                  expr='sum(kubernetes_service_operation_status_code:buildbarn_fuse_raw_file_system_operations_duration_seconds_count:irate1m{kubernetes_service=~"$kubernetes_service",operation=~"$operation",status_code=~"$status_code",%s="$cluster"}) by (%s)' % [$._config.clusterLabel, aggregation[0]],
            ...
            simpledash.heatmap(
              ...
              targets=[
                simpledash.heatmapTarget(
                  expr='sum(kubernetes_service_le_operation_status_code:buildbarn_fuse_raw_file_system_operations_duration_seconds_bucket:irate1m{kubernetes_service=~"$kubernetes_service",operation=~"$operation",status_code=~"$status_code",%s="$cluster"}) by (le)' % $._config.clusterLabel,
                ),
              ],
            ),
          ],
        ...
        simpledash.row(
          ...
          panels=[
            simpledash.graph(
              ...
              targets=[
                simpledash.graphTarget(
                  expr='sum(callback_kubernetes_service_status_code:buildbarn_fuse_raw_file_system_callbacks:irate1m{callback=~"$callback",kubernetes_service=~"$kubernetes_service",status_code=~"$status_code",%s="$cluster"}) by (%s)' % [$._config.clusterLabel, aggregation[0]],
                  legendFormat='{{%s}}' % aggregation[0],
                ),
              ],
            )
